...
def training_loop(
    ...
    training_set = generator.files.dnnlib.util.construct_class_by_name(**training_set_kwargs) # subclass of training.data.Dataset
    ...
    G = generator.files.dnnlib.util.construct_class_by_name(**G_kwargs, **common_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    D = generator.files.dnnlib.util.construct_class_by_name(**D_kwargs, **common_kwargs).train().requires_grad_(False).to(device) # subclass of torch.nn.Module
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        with generator.files.dnnlib.util.open_url(resume_pkl) as f:
    ...
    loss = generator.files.dnnlib.util.construct_class_by_name(device=device, G=G, G_ema=G_ema, D=D, **loss_kwargs) # subclass of training.loss.Loss
    phases = []
    # for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval), ('D', D, D_opt_kwargs, D_reg_interval)]:
    for name, module, opt_kwargs, reg_interval in [('G', G, G_opt_kwargs, G_reg_interval)]:
        if reg_interval is None:
            opt = generator.files.dnnlib.util.construct_class_by_name(params=module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
            ...
            opt = generator.files.dnnlib.util.construct_class_by_name(module.parameters(), **opt_kwargs) # subclass of torch.optim.Optimizer
    ...
    while True:
        ...
        fields += [f"time {generator.files.dnnlib.util.format_time(training_stats.report0('Timing/total_sec', tick_end_time - start_time)):<12s}"]
