...
import numpy as np
import torch
from generator.files import dnnlib
...
def get_feature_detector(url, device=torch.device('cpu'), num_gpus=1, rank=0, verbose=False):
    ...
    if key not in _feature_detector_cache:
        ...
        with generator.files.dnnlib.util.open_url(url, verbose=(verbose and is_leader)) as f:
...
def iterate_random_labels(opts, batch_size):
    if opts.G.c_dim == 0:
        ...
        dataset = generator.files.dnnlib.util.construct_class_by_name(**opts.dataset_kwargs)
        while True:
...
class ProgressMonitor:
    ...
    def update(self, cur_items):
        ...
        if (self.verbose) and (self.tag is not None):
            print(f'{self.tag:<19s} items {cur_items:<7d} time {generator.files.dnnlib.util.format_time(total_time):<12s} ms/item {time_per_item * 1e3:.2f}')
...
def compute_feature_stats_for_dataset(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, data_loader_kwargs=None, max_items=None, swav=False, sfid=False,  **stats_kwargs):
    ...
    print('Calculating the stats for this data the first time\n')
    ...
    progress = opts.progress.sub(tag='data features', num_items=num_items, rel_lo=rel_lo, rel_hi=rel_hi)
